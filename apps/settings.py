...
def app():
    ...
    with st.form("addSubAgent", clear_on_submit=True):
        ...
        if submitted:
            ...
            mainDBcursor.execute("insert into subagents(agency_name, tier, rocket_code, aon_code, neptune_code, palomar_code, wright_code) values(?, ?, ?, ?, ?, ?, ?)", (newAgencyName, newAgencyTier, newRocketCode, int(newAonCode), newNeptuneCode, newPalomarCode, int(newWrightCode)))
