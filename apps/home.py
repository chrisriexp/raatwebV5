...
def easyAccountingScript():
    ...
    for agency in result:
        ...
        neptuneQuery = neptuneReqDf.query("TnxAgencyNo==@agency[5] & Premium != 0 & Commission != 0")
        
        #Query to get Wright information that will append to PDF
        palomarQuery = palomarReqDf.query("`Agent Code`==@agency[6]")
        
        #Query to get Palomar information that will append to PDF
        wrightQuery = wrightReqDf.query("ProducerNo==@agency[7]")
        ...
        commTotal = round(aonComm+neptuneComm+palomarComm+wrightComm, 2)
...
def app():
    ...
    with right_column:
        ...
        if palomarExists == True & wrightExists == True & aonExists == True & neptuneExists == True:
